...
{
	MUX_VAL(CP(SYS_BOOT3),       (IEN  | PTU | EN  | M4)); /* GPIO_5 */
	MUX_VAL(CP(SYS_BOOT6),       (IEN  | PTU | EN  | M4)); /* GPIO_8 */
}
...
static void gpio_i2c_config_pin(GPIO_I2C_PIN pin, GPIO_I2C_DIRECTION dir)
{
	if (dir == GPIO_I2C_INPUT) {
		if (pin == GPIO_I2C_SCLK)
			omap_set_gpio_direction(GPIO_I2C_GPIO_SCLK, 1);
		else
			omap_set_gpio_direction(GPIO_I2C_GPIO_SDATA, 1);
	} else if (dir == GPIO_I2C_OUTPUT) {
		if (pin == GPIO_I2C_SCLK)
			omap_set_gpio_direction(GPIO_I2C_GPIO_SCLK, 0);
		else
			omap_set_gpio_direction(GPIO_I2C_GPIO_SDATA, 0);
	}
}
...
{
	if (pin == GPIO_I2C_SCLK)
		return omap_get_gpio_datain(GPIO_I2C_GPIO_SCLK);
	else
		return omap_get_gpio_datain(GPIO_I2C_GPIO_SDATA);
	return 0;
}
...
static void gpio_i2c_set_pin_level(GPIO_I2C_PIN pin, int level)
{
	...
	if (pin == GPIO_I2C_SCLK) {
		pin_level = omap_get_gpio_datain(GPIO_I2C_GPIO_SCLK);
		if (((level == 1) && (pin_level == 0)) ||
		    ((level == 0) && (pin_level == 1)))
			omap_set_gpio_dataout(GPIO_I2C_GPIO_SCLK, level);
	} else if (pin == GPIO_I2C_SDATA) {
		if (level == 0) {
			gpio_i2c_config_pin(pin, GPIO_I2C_OUTPUT);
			omap_set_gpio_dataout(GPIO_I2C_GPIO_SDATA, 0);
		} else {
...
identify_device(void)
{
	...
	if (DEBUG_PRODUCTION_DATA)
		printf("%s: Huh? couldn't ID device\n", __FUNCTION__);
...
int fetch_production_data(void)
{
	...
	if (read_user_zone(0, 0, (unsigned char *)&product_id_data.d.u_zone0, sizeof(product_id_data.d.u_zone0))) {
		printf("failed reading zone0 data!\n");
		err = -1;
		goto out;
	}
	...
	if (read_user_zone(2, 0, (unsigned char *)&product_id_data.d.wifi_config_data, sizeof(product_id_data.d.wifi_config_data))) {
...
void dump_production_data(void)
{
	...
	for (i=1; i<4; ++i) {
		if (!extract_mac_address(&product_id_data, i, mac))
			printf("LAN[%d] = %02x:%02x:%20x:%02x:%02x:%02x\n",
